...
async function loadDoctors(container, token) {
  try {
    ...
    if (response.ok) {
      ...
      doctors.forEach(doctor => {
        html += `
          <div class="doctor-card">
            ...
            <button class="btn secondary-btn book-appointment" data-id="${doctor.id}">Reservar cita</button>
          </div>
        `;
      });
      html += '</div>';
      ...
      container.innerHTML = '<p>Error al cargar los doctores. Intente nuevamente más tarde.</p>';
    }
  } catch (error) {
